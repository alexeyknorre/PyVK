...
PAJEK_EDGELIST = './results/edgelist.net'
...
columns = ["uid", "music"]
USELESS_WORDS = u'разная|разное|разные|по настроению|под настроение|меломанка|меломан|всякая|все|всякая|нету|зависит от настроения|слушаю всё|всё|нет'




def read_data(csv=csv, columns=columns):
    df = pd.read_csv(csv, na_values = ["[]"], 
                 usecols = columns, encoding="utf-8")
    df["music"] = df["music"].str.lower() 
    df = df.dropna(subset = ['music'])
    return df 
...
def get_vertices(sorted_dict, minimum_occurences):
    ...
    for i in sorted_dict:
        if i[1] >= minimum_occurences:
            vertices.append(i[0])
    vertices.remove(u'')
    return vertices

def save_edgelist(vertices,arcs):
    print "Saving..."
...
#arcs = get_arcs(df_clean, v)
#save_edgelist(v,arcs)
#show_sorted_dict(srt, elements=500)